Android Log Matcher - A Python module that matches string with logcat

Overview :

    Android Log Matcher matches a string or a regular expression pattern with
    an output of "adb logcat". For example, To check whether
    ...
    If the string appears in log, logmatcher.wait() returns True.
...
Usage :
    ...
    5. Check the return value of these method. logmatcher.wait() returns True
       if the string is found in the log. logmatcher.waitPattern() returns
       Match object if the pattern is matched in the log.
    ...
    logmatcher.waitPattern(pattern, timeout = defaultTimeout)
        ...
        Argument :
            ...
            pattern : A str or unicode that represents regular
                ...
                value that is searched the watching log.
